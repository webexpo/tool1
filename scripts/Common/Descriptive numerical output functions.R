...
#res is the data element of the fun.NDexpo output
  ...
  if (length(res[res$is.censored==0,1])==0) {
    ...
    results[8,2] <- as.character(paste(signif(100*length(res$x[res$x>c.oel])/length(res$x),2),'%',sep=' '))
    results[9,2] <- as.character(signif(mean(res$x),3))
    results[10,2] <- as.character(signif(sd(res$x),3))
    results[11,2] <- as.character(paste(signif(100*sd(res$x)/mean(res$x),3),'%',sep=' '))
...
if (length(res[res$is.censored==0,1])!=0)
  ...
  results <-data.frame(parameter=c('n','PropCensored','min',
    ...
    results[1,2] <- as.character(dim(res)[1])
    results[2,2] <- paste(as.character(signif(100*length(res[data.simply.imputed$notcensored==FALSE,1])/length(res[,1]),2)),"%",sep=" ")
