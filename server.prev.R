...
shinyServer(function(input, output) {
  ...
  output$distplot.frac <- renderPlot({
    ...
    frac.dep.vec <-100*(1-pnorm((log(Y$c.oel)-Z$mu.chain)/Z$sigma.chain))
    
    frac.dep.est <-X$frac$est
    
    ###vectors for the graphical iterations
    
    constrainedmu <-Z$mu.chain[Z$mu.chain<quantile(Z$mu.chain,0.9) &
                                 Z$mu.chain>quantile(Z$mu.chain,0.1)]
    
    constrainedsigma <-Z$sigma.chain[Z$sigma.chain<quantile(Z$sigma.chain,0.9) &
                                       Z$sigma.chain>quantile(Z$sigma.chain,0.1)]
    
    
    ###iteration corresponding to the best estimate
    
    medmu <-Z$mu.chain[which(abs(frac.dep.vec-frac.dep.est)==min(abs(frac.dep.vec-frac.dep.est)))][1]
    
    medsigma<-Z$sigma.chain[which(abs(frac.dep.vec-frac.dep.est)==min(abs(frac.dep.vec-frac.dep.est)))][1]  
  ...
  output$distplot.perc <- renderPlot({
    ...
    perc.vec <-exp(Z$mu.chain + qnorm(W$target_perc/100)*Z$sigma.chain)
    ...
    medmu <-Z$mu.chain[which(abs(perc.vec-perc.est)==min(abs(perc.vec-perc.est)))][1]
    
    medsigma<-Z$sigma.chain[which(abs(perc.vec-perc.est)==min(abs(perc.vec-perc.est)))][1]  
  ...
  output$distplot.AM <- renderPlot({
    ...
    ### exceedance chain and best estimatee
    
    ### exceedance chain and best estimate
    
    am.vec <-exp(Z$mu.chain + 0.5*Z$sigma.chain^2)
    ...
    medmu <-Z$mu.chain[which(abs(am.vec-am.est)==min(abs(am.vec-am.est)))][1]
    
    medsigma<-Z$sigma.chain[which(abs(am.vec-am.est)==min(abs(am.vec-am.est)))][1]  
