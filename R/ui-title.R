                ...
                tags$ul(
                    ...
                    tags$li(
                        ...
                        tags$ul(
                            ...
                            tags$li(
                                shiny::actionButton(
                                    inputId = ns("btn_lang_ko"),
                                    class   = "dropdown-item fs-6",
                                    label   = default_lang_names[["ko"]]
                                )
                            ),
...
server_title <- function(id) {
    server <- \(input, output, session) {
        ...
        shiny::observe(priority = 10L, {
            ...
            shinyjs::removeClass(sprintf("btn_lang_%s", old_lang), "active")
            shinyjs::addClass("btn_lang_ko", "active")

            update_query_string(lang = set_lang("ko"))
        }) |>
        shiny::bindEvent(input$btn_lang_ko, ignoreInit = TRUE)
